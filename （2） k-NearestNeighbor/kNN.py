...
from numpy import *  #NameError: name 'zeros' is not defined
import operator
...
#归一化数据 ：归一化就是要把你需要处理的数据经过处理后（通过某种算法）限制在你需要的一定范围内。首先归一化是为了后面数据处理的方便，其次是保正程序运行时收敛加快。
...
# 训练算法：此步骤不适用于 k-近邻算法：因为测试数据每一次都要与全量的训练数据进行比较，所以这个过程是没有必要的。
